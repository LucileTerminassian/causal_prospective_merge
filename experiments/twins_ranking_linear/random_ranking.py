...
with warnings.catch_warnings():
    ...
    for i in tqdm(range(cfg["num_repeats"])):
        # try:
            ...
            candidate_sites = generating_random_sites_from(XandT, data_with_groundtruth, exp_parameters, added_T_coef=added_T_coef, binary_outcome=binary_outcome)
            ...
            for i, candidate in merged_datasets.items():
                ...
                if cfg["model_param"]["GP"]["run"]:
                    ...
                    cGP_new = CausalGP(max_gp_iterations = max_gp_iterations_test,mode=gp_mode)
