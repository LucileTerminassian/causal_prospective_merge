...
def compare_obs_sum_EIG_lin(X_host,X_cand,cov,DP=False,epsilon=0.01):
        ...
        if DP:
            EIG_DP = get_diff_private_sum_obs_EIG(X_host,X_cand,np.array(cov),epsilon=epsilon)
            results_dict["EIG_obs_DP"] = EIG_DP
...
def compare_caus_EIG_lin(X_host,X_cand,cov,causal_param_first_index,DP=False,epsilon=0.01):
        ...
        EIG_crypten = logdet_Crypten(XX_cand_crypten @ X_host_crypten + torch.eye(X_host_plus_cov_inv.shape[0]))
        # EIG_crypten = EIG_crypten + (len(X_host_crypten))*np.log(len(X_host_crypten))
        # EIG_crypten = EIG_crypten.get_plain_text()
...
def compare_caus_sum_EIG_lin(X_host,X_cand,cov,causal_param_first_index,DP=False,epsilon=0.01):
        results_dict = {}
        ...
        if DP:
            EIG_DP = get_diff_private_sum_caus_EIG(X_host,X_cand,np.array(cov),causal_param_first_index,epsilon=epsilon)
            results_dict["EIG_caus_DP"] = EIG_DP
...
def get_diff_private_sum_caus_EIG(X_host,X_cand,cov,causal_param_first_index,epsilon=0.01):
