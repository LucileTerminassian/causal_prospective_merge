...
main_dir = notebook_dir[
    : notebook_dir.find("causal_prospective_merge") + len("causal_prospective_merge")
]
...
exp_parameters = {
    "number_of_candidate_sites": number_of_candidate_sites + 1,
...
max_gp_iterations_test = cfg["model_param"]["GP"]["max_gp_iterations_test"]
...
with warnings.catch_warnings():
    ...
    for i in tqdm(range(cfg["num_repeats"])):
        # try:
        XandT = XandT.sample(
            n=desired_initial_sample_size, replace=True, random_state=42
        )
        # dictionnary of random sites
        candidate_sites = generating_random_sites_from(
            XandT,
            data_with_groundtruth,
            exp_parameters,
            added_T_coef=added_T_coef,
        ...
        if data_name == "twins":
            ...
            for i, cand in candidate_sites.items():
                ...
                candidate_sites[i]["Y"] += std_true_y * np.random.randn(
                    len(candidate_sites[i]["Y"])
                )

        host = candidate_sites[0][:host_sample_size]
        XandT_host, Y_host = torch.from_numpy(
            host.drop(columns=["Y"]).values
        ), torch.from_numpy(host["Y"].values)

        host_test = candidate_sites[0][
            host_sample_size:
        ]  # will be of size host_test_size
        XandT_host_test, Y_host_test = torch.from_numpy(
            host_test.drop(columns=["Y"]).values
        ), torch.from_numpy(host_test["Y"].values)

        candidate_sites = {
            key: value for key, value in candidate_sites.items() if key != 0
        }
        ...
        if cfg["model_param"]["GP"]["run"]:
            ...
            X_host = (
                host.drop(columns=["Y", "T"]).iloc[:, :causal_param_first_index].values
            )
        ...
        if cfg["model_param"]["BART"]["run"]:
            ...
            bcf = BayesianCausalForest(
                prior_hyperparameters=cfg["model_param"]["BART"][
                    "prior_hyperparameters"
                ],
                predictive_model_parameters=cfg["model_param"]["BART"][
                    "predictive_model_parameters"
                ],
                conditional_model_param=cfg["model_param"]["BART"][
                    "conditional_model_param"
                ],
            )

            X_host = (
                host.drop(columns=["Y", "T"])
                .iloc[:, :causal_param_first_index]
        ...
        for _, candidate in candidate_sites.items():
            ...
            if cfg["model_param"]["BART"]["run"]:
                ...
                result = bcf.joint_EIG_calc_eff(
                    X_cand,
                    T_cand.astype(np.int32),
                    sampling_parameters=cfg["model_param"]["BART"][
                        "sampling_parameters"
                    ],
                )
                eig_results["EIG_obs_bart"].append(result["Obs EIG"])
                eig_results["EIG_caus_bart"].append(result["Causal EIG"])
        ...
        X_one.iloc[:, causal_param_first_index:] = XandT_host_test.iloc[
        ...
        if data_name == "twins":  # binary outcome
            ...
            true_cate = (
                data_with_groundtruth["y1"].iloc[indices_list]
                - data_with_groundtruth["y0"].iloc[indices_list]
            )
        ...
        for i, candidate in merged_datasets.items():
            ...
            if cfg["model_param"]["Linear"]["run"]:
                ...
                merged_mse_models["lin"] = merged_mse_models.get("lin", []) + [
                    (mean_squared_error(true_cate, pred_cate))
                ]

            if cfg["model_param"]["BART"]["run"]:

                X_host_test = (
                    host_test.drop(columns=["Y", "T"])
                    .iloc[:, :causal_param_first_index]
                    .values
                )
                ...
                merged_mse_models["BART"] = merged_mse_models.get("BART", []) + [
                    (mean_squared_error(true_cate, pred_cate))
                ]
            ...
            if cfg["model_param"]["GP"]["run"]:
                ...
                cGP_new = CausalGP(
                    max_gp_iterations=max_gp_iterations_test, mode=gp_mode
                )
                ...
                merged_mse_models["GP"] = merged_mse_models.get("GP", []) + [
                    (mean_squared_error(true_cate, pred_cate))
                ]

        if cfg["model_param"]["Linear"]["run"]:

            obs_eig_ranking_closed_form = sorted(
                range(len(eig_results[" EIG_obs_lin"])),
                key=lambda i: eig_results[" EIG_obs_lin"][i],
                reverse=True,
            )

            caus_eig_ranking_closed_form = sorted(
                range(len(eig_results[" EIG_caus_lin"])),
                key=lambda i: eig_results[" EIG_caus_lin"][i],
                reverse=True,
            )
            ...
            true_cate_ranking = sorted(
                range(len(merged_mse_models["lin"])),
                key=lambda i: merged_mse_models["lin"][i],
                reverse=False,
            )
            ...
            compare_to_ground_truth(
                correlation_with_true_rankings["lin"],
                true_cate_ranking,
                obs_eig_ranking_closed_form,
                merged_mse=merged_mse_models["lin"],
                top_n=top_n,
                k=k,
            )
            compare_to_ground_truth(
            ...
            ### random ranking
            random_ranking = np.random.choice(
                np.arange(1, number_of_candidate_sites + 1),
                size=number_of_candidate_sites,
                replace=False,
            )

            ### ranking by sample size
            sample_size_order = sorted(
                candidate_sites.keys(), key=lambda key: -candidate_sites[key].shape[0]
            )
            ...
            mvn = multivariate_normal(
                mean=mean_vector_host, cov=cov_matrix_host, allow_singular=1
            )
            # get log likelihood of candidate sites
            log_likelihood_list = []
            for i, candidate in candidate_sites.items():
                log_likelihoods = mvn.logpdf(
                    candidate.iloc[:, :causal_param_first_index].values
                )
                log_likelihood_list.append(np.mean(log_likelihoods))

            similarity_cov_distrib_ranking = sorted(
                range(len(log_likelihood_list)),
                key=lambda i: log_likelihood_list[i],
                reverse=True,
            )
            ...
            t_host_pred = ps_model.predict(
                XandT_host.iloc[:, :causal_param_first_index]
            )
            ...
            for i, candidate in candidate_sites.items():
                ...
                t_cand_pred = ps_model.predict(
                    candidate.iloc[:, :causal_param_first_index]
                )  # predict on host!
                mse_cand = abs(
                    mean_squared_error(t_cand_pred, candidate["T"]) - mse_host
                )
                mse_diff_list.append(mse_cand)

            similarity_pscore_ranking = sorted(
                range(len(mse_diff_list)), key=lambda i: mse_diff_list[i], reverse=True
            )
            ...
            compare_to_ground_truth(
                correlation_with_true_rankings["lin"],
                true_cate_ranking,
                list(similarity_cov_distrib_ranking),
                merged_mse=merged_mse_models["lin"],
                top_n=top_n,
                k=k,
            )
            compare_to_ground_truth(
                correlation_with_true_rankings["lin"],
                true_cate_ranking,
                list(similarity_pscore_ranking),
                merged_mse=merged_mse_models["lin"],
                top_n=top_n,
                k=k,
            )
            correlation_with_true_rankings["lin"][
                "Method"
            ] = correlation_with_true_rankings["lin"].get("Method", []) + [
                "obs_closed_form",
                "caus_closed_form",
                "random",
                "sample size",
                "similarity_cov_distrib_ranking",
                "similarity_pscore_ranking size",
            ]

        if cfg["model_param"]["GP"]["run"]:

            obs_eig_ranking_closed_form = sorted(
                range(len(eig_results["EIG_obs_GP"])),
                key=lambda i: eig_results["EIG_obs_GP"][i],
                reverse=True,
            )

            caus_eig_ranking_closed_form = sorted(
                range(len(eig_results["EIG_caus_GP"])),
                key=lambda i: eig_results["EIG_caus_GP"][i],
                reverse=True,
            )
            ...
            true_cate_ranking = sorted(
                range(len(merged_mse_models["GP"])),
                key=lambda i: merged_mse_models["GP"][i],
                reverse=False,
            )
            ...
            compare_to_ground_truth(
                ...
                true_cate_ranking,
                obs_eig_ranking_closed_form,
                merged_mse=merged_mse_models["GP"],
                top_n=top_n,
                k=k,
            )
            compare_to_ground_truth(
            ...
            correlation_with_true_rankings["GP"][
                "Method"
            ] = correlation_with_true_rankings["GP"].get("Method", []) + [
                "obs_closed_form",
                "caus_closed_form",
                "random",
                "sample size",
        ...
        if cfg["model_param"]["BART"]["run"]:
            ...
            obs_eig_ranking_closed_form = sorted(
                range(len(eig_results["EIG_obs_bart"])),
                key=lambda i: eig_results["EIG_obs_bart"][i],
                reverse=True,
            )

            caus_eig_ranking_closed_form = sorted(
                range(len(eig_results["EIG_caus_bart"])),
                key=lambda i: eig_results["EIG_caus_bart"][i],
                reverse=True,
            )
            ...
            true_cate_ranking = sorted(
                range(len(merged_mse_models["BART"])),
                key=lambda i: merged_mse_models["BART"][i],
                reverse=False,
            )
            ...
            compare_to_ground_truth(
                ...
                true_cate_ranking,
                obs_eig_ranking_closed_form,
                merged_mse=merged_mse_models["BART"],
                top_n=top_n,
                k=k,
            )
            compare_to_ground_truth(
            ...
            correlation_with_true_rankings["BART"][
                "Method"
            ] = correlation_with_true_rankings["BART"].get("Method", []) + [
                "obs_closed_form",
                "caus_closed_form",
                "random",
                "sample size",
