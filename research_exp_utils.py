...
def eig_closed_form_varying_sample_size(
    host_mirror_generation_function,
    ...
    for length in n_both_candidates_list:
        ...
        design_data_host, design_data_mirror = host_mirror_generation_function(
            X_rct,
            T_rct,
            p_assigned_to_cand2,
            n_host,
        ...
        EIG_obs_one_n_mirror = compute_EIG_obs_closed_form(
            X_mirror_arr, cov_matrix_post_host, sigma_rand_error
        )
        EIG_obs_one_n_cand2 = compute_EIG_obs_closed_form(
            X_cand2_arr, cov_matrix_post_host, sigma_rand_error
        )

        EIG_obs_closed_form_mirror.append(EIG_obs_one_n_mirror)
        EIG_obs_closed_form_cand2.append(EIG_obs_one_n_cand2)

        EIG_caus_one_n_mirror = compute_EIG_causal_closed_form(
            X_mirror_arr,
            cov_matrix_post_host,
            sigma_rand_error,
            causal_param_first_index,
        )
        EIG_caus_one_n_cand2 = compute_EIG_causal_closed_form(
            X_cand2_arr,
            cov_matrix_post_host,
            sigma_rand_error,
            causal_param_first_index,
...
def eig_from_samples_varying_sample_size(
    n_both_candidates_list,
    n_rct_before_split,
    x_distributions,
    p_assigned_to_cand2,
    ...
    for length in n_both_candidates_list:
        ...
        EIG_obs_one_n_mirror = compute_EIG_obs_from_samples(
            pred_list_mirror, sigma_rand_error
        )
        EIG_obs_one_n_cand2 = compute_EIG_obs_from_samples(
            pred_list_cand2, sigma_rand_error
        )
        ...
        pred_in_causal_form_mirror = predictions_in_EIG_causal_form(
            pred_func,
            n_non_causal_expectation,
            causal_param_first_index,
            theta_samples=beta_post_host.values[:40],
            theta_sampling_function=sample_func,
        )
        pred_in_causal_form_cand2 = predictions_in_EIG_causal_form(
            pred_func,
            n_non_causal_expectation,
            causal_param_first_index,
            theta_samples=beta_post_host.values[:40],
        ...
        EIG_caus_one_n_mirror = compute_EIG_causal_from_samples(
            pred_list_mirror, pred_in_causal_form_mirror, sigma_rand_error
        )
        EIG_caus_one_n_cand2 = compute_EIG_causal_from_samples(
            pred_list_cand2, pred_in_causal_form_cand2, sigma_rand_error
        )
