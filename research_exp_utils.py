...
def bart_eig_from_samples_varying_sample_size(
    ...
    for length in sample_sizes:
        ...
        for cand in candidates_names:
            ...
            joint_eig = bcf.joint_EIG_calc(X_cand_not_T, T_cand, sampling_parameters)
...
def mean_average_precision(true_rankings, predicted_rankings, k=None):
    ...
    avg_precision = np.mean(
        [
            average_precision_at_k(true_rankings, predicted_rankings, k_)
...
def compare_to_ground_truth(
    results_dict, true_cate_ranking, eig_ranking, merged_mse, top_n=None, k=None
):
    ...
    if top_n is not None:
        ...
        topn_eig_ranking, topn_true_cate_ranking, topn_merged_mse = (
            eig_ranking,
            true_cate_ranking,
            merged_mse,
        )
    ...
    implied_ranking = [
        eig_ranking.index(val)
        for val in list(range(min(eig_ranking), max(eig_ranking) + 1))
    ]

    if type((kendalltau(implied_ranking, merged_mse)[0])) is float:

        results_dict["tau"] = results_dict.get("tau", []) + [
            (kendalltau(implied_ranking, merged_mse)[0])
        ]
        results_dict["rho"] = results_dict.get("rho", []) + [
            (spearmanr(implied_ranking, merged_mse)[0])
        ]
    else:
        results_dict["tau"] = results_dict.get("tau", []) + [
            (kendalltau(implied_ranking, merged_mse)[0]).item()
        ]
        results_dict["rho"] = results_dict.get("rho", []) + [
            (spearmanr(implied_ranking, merged_mse)[0]).item()
        ]
    if type(k) == int:
        results_dict["precision_at_k"] = results_dict.get("precision_at_k", []) + [
            precision_at_k(true_cate_ranking, topn_eig_ranking, k=k)
        ]
    else:
        for val in k:
            results_dict["precision_at_" + str(val)] = results_dict.get(
                "precision_at_" + str(val), []
            ) + [precision_at_k(true_cate_ranking, topn_eig_ranking, k=val)]
    ...
    for label, arr in data_dict.items():
        ...
        if label.endswith("mcmc"):
            ...
            x_interp = np.linspace(
                min(x), max(x), num=len(mean_data) * 3
